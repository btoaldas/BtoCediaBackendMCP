Aquí tienes el código Python completo con la función `validate_email` agregada. Se adaptó `ValidationResult` para que el campo `strength` sea opcional (ya que el correo evalúa validez de formato y no robustez) y se incorporaron validaciones conforme a los estándares RFC (longitudes, parte local, estructura de dominio y TLD), junto con su bloque de pruebas:
