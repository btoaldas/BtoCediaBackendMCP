Aquí tienes el código Python completo actualizado.

### Cambios principales aplicados:
1. **`User`**: Se agregó el atributo `is_admin: bool = False` y se asignó un valor por defecto a `password: str = ""` para permitir administradores sin contraseña (por ejemplo, autenticados vía SSO, OAuth o claves SSH).
2. **`PasswordPolicy`**: Se incorporó el flag `validate_admin_password: bool = False`, permitiendo configurar centralmente si los administradores deben o no validar contraseña.
3. **`validate_user` / `validate_users`**: Si `is_admin=True` y la validación de administradores no está activada:
   - Si no se proporciona contraseña, se marca como válida sin requerir caracteres (`strength="Opcional (Sin contraseña)"`).
   - Si se proporciona una contraseña, se calcula su fortaleza de forma informativa (`strength=...`), pero las restricciones de política no invalidan al usuario administrador.
4. **Casos de prueba (`__main__`)**: Se actualizaron y añadieron usuarios administradores (con contraseña segura, con contraseña básica y sin contraseña) junto a los usuarios regulares para comprobar el comportamiento.
...
class PasswordPolicy:
  ...
  validate_admin_password: bool = False  # Si es False, la validación de contraseña es opcional para admins
...
class ValidationResult:
  ...
  is_valid: bool
  errors: list[str]
  strength: str | None = None  # 'Débil', 'Media', 'Fuerte' o estado informativo


@dataclass
class User:
  """Representa a un usuario con sus credenciales básicas y rol administrativo."""
...
def validate_user(
    ...
    validate_admin_password: bool | None = None,
) -> UserValidationResult:
  """Valida las credenciales individuales de un usuario.

  Para usuarios con is_admin=True, la validación de contraseña es opcional
  (desactivada por defecto, configurable mediante validate_admin_password o en
  PasswordPolicy).
  """
  if policy is None:
    policy = PasswordPolicy()

  enforce_admin_password = (
      validate_admin_password
      if validate_admin_password is not None
      else policy.validate_admin_password
  )
  ...
  # Validación de contraseña: si es admin y no se exige validación, es opcional
  if user.is_admin and not enforce_admin_password:
    raw_pwd = user.password or ""
    if not raw_pwd:
      pwd_res = ValidationResult(
          is_valid=True,
          errors=[],
          strength="Opcional (Sin contraseña)",
      )
    else:
      # Si se proporciona contraseña, se evalúa fortaleza de modo informativo sin invalidar
      info_res = validate_password(raw_pwd, policy)
      pwd_res = ValidationResult(
          is_valid=True,
          errors=[],
          strength=f"{info_res.strength} (Opcional - Admin)",
      )
  else:
    pwd_res = validate_password(user.password or "", policy)
...
if __name__ == "__main__":
  # Lista de usuarios de prueba cubriendo escenarios con administradores y usuarios estándar
  users_batch = [
      User(
          username="alberto_adm",
          email="alberto@empresa.com",
          password="K9#mQ!9vLp@4WxZ",
          is_admin=True,  # Admin con contraseña fuerte -> Válido
      ),
      User(
          username="admin_sso",
          email="sso.admin@empresa.com",
          password="",  # Admin sin contraseña -> Válido (opcional para admins)
          is_admin=True,
      ),
      User(
          username="admin_temporal",
          email="admin.temp@empresa.com",
          password="admin",  # Admin con contraseña simple -> Válido (no bloquea al admin)
          is_admin=True,
      ),
      User(
          username="maria_dev",
          email="maria.gonzalez@dominio.org",
          password="Temp2024!",  # Usuario estándar con contraseña válida -> Válido
          is_admin=False,
      ),
      User(
          username="carlos_qa",
          email="carlos..invalido@dominio.com",  # Email inválido
          password="Password123!",  # Contraseña inválida (patrón prohibido)
          is_admin=False,
      ),
      User(
          username="lucia_ops",
          email="lucia@sub.empresa.co.uk",
          password="corta",  # Usuario estándar con contraseña corta -> Inválido
          is_admin=False,
      ),
      User(
          username="usuario_duplicado",
          email="alberto@empresa.com",  # Correo duplicado respecto a alberto_adm
  ...
  for idx, res in enumerate(validation_results, start=1):
    ...
    role_label = " [ADMIN]" if u.is_admin else " [ESTÁNDAR]"
    print(
        f"[{idx}] Usuario: '{u.username}'{role_label} | Estado General:"
        f" {status_label}"
    )
    print(
        f"    - Email:    '{u.email}' ->"
        f" {'✓ OK' if res.email_result.is_valid else '✗ ERROR'}"
    )
