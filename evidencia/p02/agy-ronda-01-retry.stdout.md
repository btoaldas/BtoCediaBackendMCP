Aquí tienes un validador de contraseñas modular, configurable y listo para producción en Python (3.10+). Incluye tipado estático, reglas comunes de robustez, detección de patrones inseguros y estimación de fuerza.
...
1. **Configuración declarativa (`PasswordPolicy`)**: permite ajustar longitudes, requerimientos y caracteres permitidos sin tocar la lógica.
2. **Lista de exclusión básica**: detecta subcadenas habituales (`admin`, `123456`, `password`).
3. **Respuesta estructurada (`ValidationResult`)**: expone `is_valid`, la lista explícita de `errors` para retroalimentar al usuario, y un indicador de `strength`.
